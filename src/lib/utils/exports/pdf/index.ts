...
export async function generateDailyAttendanceReportPDF(
	props: DocumentMetaDetails
): Promise<HelperResponse<string | null>> {
	const { info, event_details, participants } = props;
	...
	if (!participants || participants.length === 0) {
		return {
			status: 404,
			message: 'No participants found to generate attendance report',
			data: null
		};
	}
	try {
		...
		const participant_attendance = participants
			.map((participant) => {
		...
		console.log(
			`Participant Attendance Records: ${JSON.stringify(participant_attendance, null, 2)}`
		);

		const current_event_day = event_details
			? getEventDayInfo(
		...
		const currentEventDate =
			event_details && current_event_day && !isNaN(parseInt(current_event_day))
				? new Date(
						new Date(event_details.start_date).getTime() +
							(parseInt(current_event_day) - 1) * 24 * 60 * 60 * 1000
					)
				: new Date();
		...
		const tableBody = [
			...
			...participant_attendance.map((participant) => {
				...
				return [
					...
					{
						text: participant.am_time_in
							? formatDateToTimeOption(new Date(participant.am_time_in))
							: 'N/A'
					},
					{
						text: participant.am_time_out
							? formatDateToTimeOption(new Date(participant.am_time_out))
							: 'N/A'
					},
					{
						text: participant.pm_time_in
							? formatDateToTimeOption(new Date(participant.pm_time_in))
							: 'N/A'
					},
					{
						text: participant.pm_time_out
							? formatDateToTimeOption(new Date(participant.pm_time_out))
							: 'N/A'
					},
...
export async function generateFullEventAttendanceReportPDF(
	...
	try {
		...
		console.log(
			`Found ${attendance_records.length} attendance records for event`,
			attendance_records
		);
		...
		attendance_records.forEach((record) => {
			...
			if (!isNaN(dayNumber) && attendanceByDay.has(dayNumber)) {
				console.log(
					`Processing record for day ${dayNumber}, participant ${participantId}:`,
					record
				);
		...
		for (let day = 1; day <= totalDays; day++) {
			...
			const dayParticipantAttendance = participants
				.map((participant) => {
					...
					if (attendance) {
						console.log(
							`Found attendance for ${participant.first_name} ${participant.last_name}:`,
			...
			const absentCount = day_event_status === 'completed' ? participants.length - presentCount : 0; // Don't count absences for ongoing or upcoming days
