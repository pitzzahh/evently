<script lang="ts" generics="TData, TValue, FilterType = string">
	...
	type ComponentState = {
		...
		loaded_data: number;
		all_items: DataTableFacetedFilterProps<
			TData,
			TValue,
			FilterType
		>["options"];
	...
	const selectedValues = $derived(
		new SvelteSet(column?.getFilterValue() as FilterType[]),
	);
				...
				<Command.Group>
					<div bind:this={comp_state.root_element}>
						...
						>
							...
							{#snippet coolingOff()}
								<span
									class="text-xs text-muted-foreground max-w-[10rem] break-words text-center"
									>Loop detected while loading more {title}, fixing... please
									wait and try again</span
								>
							{/snippet}
						</InfiniteLoader>
					</div>
