...
<div class="calendar">
	{#each headers as header}
		<button class="day-name" onclick={() => headerClick(header)}>
			{header}
		</button>
	{/each}

	{#each days as day}
		<button class="day {day.enabled ? '' : 'day-disabled'}" onclick={() => dayClick(day)}>
			{day.name}
		</button>
	{/each}
	...
	{#each items as item}
		<button
			...
			style="grid-column: {item.startCol} / button {item.len};
